        ...
        match texture_id {
            ...
            egui::TextureId::User(id) => {
                ...
                if id < self.user_textures.len() {
                    let user_texture = self.user_textures[id].as_ref()?;
            ...
            egui::TextureId::User(id) => {
                ...
                if id < self.user_textures.len() {
                    if let Some(user_textures) = self.user_textures[id].as_mut() {
                        user_textures.pixels = Vec::with_capacity(pixels.len() * 4);
                        for p in pixels {
                            user_textures.pixels.push(p[0]);
                            user_textures.pixels.push(p[1]);
